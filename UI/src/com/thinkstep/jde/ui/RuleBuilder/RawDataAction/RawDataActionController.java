 ...
 * @author forell
 */
public class RawDataActionController implements Initializable {

    
    
    private RuilBuilderRawDataValueActionCallback rb;
    ...
    private void btnClose(ActionEvent event) {
        if (createActionInstance()){
            rb.setValueAction(action);
            Stage stage = (Stage) this.cbDataKey.getScene().getWindow();
            stage.close();
        }
    }
    
    public void setRuleBulderConstantAction(RuilBuilderRawDataValueActionCallback rb){
        this.rb = rb;
    }
    
    public void setAction(AssignValueAction action){
        this.action = action;
        NumberFormat nf = NumberFormat.getInstance();
        this.cbDataKey.setValue(action.getKey());
